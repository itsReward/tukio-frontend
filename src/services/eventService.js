// src/services/eventService.js - Fixed attendance endpoint handling
import api from './api';
...
class EventService {
     ...
     * @param {string|number} eventId Event ID
     */
    async getMyAttendance(eventId, userId ) {
        try {
            console.log('=== FETCHING MY ATTENDANCE ===');
            console.log('Event ID:', eventId);
            console.log('Request URL:', `${EVENT_ENDPOINTS.EVENTS}/${eventId}/attendance/${userId}`);

            const response = await api.get(`${EVENT_ENDPOINTS.EVENTS}/${eventId}/attendance/${userId}`);

            console.log('My attendance response:', response.data);
            return response;
        } catch (error) {
            console.log('No attendance record found:', error.message);
            throw error; // Re-throw so calling code can handle
        }
    }

    /**
     * Get current user's attendance record
     * @param  {string|number} userId User ID
     */
    async getMyAttendedEvents( userId ) {
        try {
            console.log('=== FETCHING MY ATTENDANCE ===');
            console.log('User ID:', userId);
            console.log('Request URL:', `${EVENT_ENDPOINTS.EVENTS}/user/${userId}/attended`);

            const response = await api.get(`${EVENT_ENDPOINTS.EVENTS}/user/${userId}/attended`);
     ...
     * @param {string|number} eventId Event ID
     */
    async getMyRating(eventId,userId) {
        try {
            console.log('=== FETCHING MY RATING ===');
            console.log('Event ID:', eventId);

            const response = await api.get(`${EVENT_ENDPOINTS.EVENTS}/${eventId}/rating/${userId}`);

            console.log('My rating response:', response.data);
            return response;
        } catch (error) {
            console.log('No rating found:', error.message);
            throw error; // Re-throw so calling code can handle
        }
    }
